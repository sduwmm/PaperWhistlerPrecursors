...
% TintInterested=irf.tint('2017-12-17T17:53:22.000Z/2017-12-17T17:53:22.500Z');
% % % %case#3
% caseCutBrst=1;
...
% TintInterested=irf.tint('2017-12-29T19:11:41.000Z/2017-12-29T19:11:42.000Z');%%%>0.8%%%%选用这个！！！！！
% % % % %case#5
% caseCutBrst=1;
...
% TintInterested=irf.tint('2018-01-09T08:34:52.000Z/2018-01-09T08:34:53.000Z');
...
% % % %case#8
% % % % demo case
%%%%%crib  Bsumthres=3*1e-2;
% caseCutBrst=2;
% trangeLoadData=('2018-12-10T04:18:30.000Z/2018-12-10T04:20:20.000Z');
% TintShow=irf.tint('2018-12-10T04:20:00.000Z/2018-12-10T04:20:06.000Z');
% TintEBSP=irf.tint('2018-12-10T04:19:45.000Z/2018-12-10T04:20:25.000Z');
% %
% tShock=irf.time_array('2018-12-10T04:20:02.200Z');
...
% TintInterested=irf.tint('2018-12-10T06:28:57.000Z/2018-12-10T06:28:59.000Z');%%%%选择这个时间段
...
% TintInterested=irf.tint('2019-12-06T05:13:57.000Z/2019-12-06T05:14:00.000Z');
% % % % case#14
% caseCutBrst=1;
...
% TintInterested=irf.tint('2021-03-05T20:27:09.000Z/2021-03-05T20:27:10.000Z');
...
% TintInterested=irf.tint('2021-03-20T19:29:42.000Z/2021-03-20T19:29:47.000Z');
...
%
%%
aimTime=find(time >= epochUnixInterested(1) & time <= epochUnixInterested(2));
aimFreq=find(frequency >= freInterested(1) & frequency <= freInterested(2));
thetaKBAim=thetak(aimTime,aimFreq);
BsumAim=Bsum(aimTime,aimFreq);
%thetaKBSVD=mean(mean(thetaKBaim,'omitnan'),'omitnan');
% power-weighted
TF=isnan(thetaKBAim);
BsumAim(TF)=nan;
stepOne=BsumAim.*thetaKBAim;
thetaKBSVD=sum(stepOne,'all','omitnan')/sum(BsumAim,'all','omitnan');
%%
%{
% Remove points with very low B amplitutes
%Bsum < Bsumthres |

removepts = find(frequency <= freInterested(1) | frequency >= freInterested(2) | planarity < planthres);
...
irf_legend(hFrst(ip),'f_{lh}',[0.93 0.55],'color','w','fontsize',22,'FontWeight','bold');
hold(hFrst(ip),'off');
set(hFrst(ip),'yscale','log');
