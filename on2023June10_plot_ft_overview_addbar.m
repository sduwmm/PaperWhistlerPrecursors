...
arrBt=BtSRVY.data;
movmeanWnd=10;
arrBxyzMovmean=movmean(arrBxyz,movmeanWnd,1);%%%%滑动窗口可以选择
arrBtMovmean=movmean(arrBt,movmeanWnd,1);
arrNeMovmean=movmean(arrNe,movmeanWnd,1);
BxyzBKG=TSeries(BxyzSRVY.time,arrBxyzMovmean);
NeBKG=TSeries(BxyzSRVY.time,arrNeMovmean);
%
...
%polarization = irf_ebsp(ExyzFAST,BxyzBRST,[],BxyzBKG,Rxyz,[],'polarization','fac');
polarization = irf_ebsp(ExyzBRST,BxyzBRST,[],BxyzBKG,Rxyz,[0.5,100],'polarization','fac');%%%%0.5-100[0.01,100]
%polarization = irf_ebsp(ExyzBRST,BscmBRST,[],BxyzBKG,Rxyz,[],'polarization','fac');
%polarization = irf_ebsp(ExyzBRST,tsBgseFSM,[],BxyzBKG,Rxyz,[],'polarization','fac');
...
hold(h(ip),'on');
irf_plot(h(ip),Flh,'linewidth',1.5,'color','k')
irf_legend(h(ip),'f_{lh}',[0.9 0.90],'color','k','fontsize',20);
hold(h(ip),'off');
set(h(ip),'yscale','log');
